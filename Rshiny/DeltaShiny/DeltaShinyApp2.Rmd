...
#               fluidRow(box(column(10, offset = 2.5, leaflet::leafletOutput("BayDelta", width = 1400, height = 800)),  
...
server <- function(input, output, session) {
  ...
  selected_marker <- reactiveVal(NULL)
    ...
    if (is.null(id) && is.null(event$group) && is.null(selected_marker())){
      ...
      output$Station_data <- renderDataTable(NULL)
    ...
    #If Region is selected, Region + Graph
      ...
      output$Region_Click <- renderText({
      paste("Station ", fish_zoop_data()$Station[1], "lies in the ", fish_zoop_data()$Region[1], "region")
      })
      output$plot <- renderPlot(NULL)
