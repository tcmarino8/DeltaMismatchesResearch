...
unique(MyZoops$Source)
unique(MyZoops$Year)

######## MESO VS MICRO == SIZE CLASSIFICATION #######
...
###### DATA IS CLEANED AND READY TO BE INTERPOLATED AT THIS POINT ######
```
...
##### LOESS DONE FOR EACH SPECIES, STATION AND ORDINAL DAY #####
...
mutate(CPUE_pred0 = ifelse(CPUE_pred<0, 0, CPUE_pred)) %>% 
  ...
  mutate(CPUE_tot = sum(CPUE_pred0)) %>%                       #####SHOULD RENAME THIS TO CPUE_PRED_TOT
  
  ######## I CAN ADD SOMETHING HERE TO CALCULATE PERCENTS #######
  ungroup() %>% 
...
## Species pilots
unique(Meso_loess_pred3$Species)
# Eury example
...
loess_pred_eury <- ggplot(Meso_loess_pred_Eury, aes(Date, CPUE_pred0))+
...
loess_pred_pseudo <- ggplot(Meso_loess_pred_Pseudo, aes(Date, CPUE_pred0))+
  geom_vline(aes(xintercept=Species_year3), color="gray50")+
  geom_line(col="black", linewidth=1)+
...
loess_pred_Limno <- ggplot(Meso_loess_pred_Limno, aes(Date, CPUE_pred0))+
  geom_vline(aes(xintercept=Species_year3), color="gray50")+
  geom_line(col="black", linewidth=1)+
...
loess_pred_Sino <- ggplot(Meso_loess_pred_Sino, aes(Date, CPUE_pred0))+
  geom_vline(aes(xintercept=Species_year3), color="gray50")+
  geom_line(col="black", linewidth=1)+
...
loess_pred_Bos <- ggplot(Meso_loess_pred_Bos, aes(Date, CPUE_pred0))+
  geom_vline(aes(xintercept=Species_year3), color="gray50")+
  geom_line(col="black", linewidth=1)+
...
#### SEEMS TO BE AN ERROR IN THE SCALE OF DIAPTOMOS ####
...
```{r - TYLER TRIES TO PLOT}

patchwork_side <-c()
for (species in unique(Meso_pred_quantiles$Species)) {
  for (station in unique(Meso_pred_quantiles$Station)) {
    plot_df <- Meso_pred_quantiles %>% filter(Species == species, Station == station)
    hist_plot_side <- plot_df %>% ggplot(aes(Julian_year2, fill = Quantile)) + 
    geom_histogram(position = "dodge", binwidth=1)+
    labs(subtitle = paste0(species, " at Station: ",station, " predicted Julian date per quantile"))
    patchwork_side[[paste0(species, station)]] <- hist_plot_side 
  }
  
}
patchwork_side <- wrap_plots(patchwork_side, ncol = 2)

patchwork_stacked <-c()
for (species in unique(Meso_pred_quantiles$Species)) {
  plot_df <- Meso_pred_quantiles %>% filter(Species == species)
  hist_plot_stacked <- plot_df %>% ggplot(aes(Julian_year2, fill = Quantile)) + 
  geom_histogram(position = "stack", binwidth=2) +
  labs(title = species)
  patchwork_stacked[[species]] <- hist_plot_stacked
  
}
patchwork_stacked <- wrap_plots(patchwork_stacked, ncol = 2)

###DOT PLOT< BUT PROBLEM IS ALL THE STATIONS
patchwork_dot <-c()
for (species in unique(Meso_pred_quantiles$Species)) {
  plot_df <- Meso_pred_quantiles %>% filter(Species == species)
  dot_plot <- plot_df %>% ggplot(aes(x=Species_year2 , y=Julian_year2))+
        geom_point(aes(color=Quantile, shape = Station), size=.5)
  patchwork_dot[[species]] <- dot_plot
}
patchwork_dot <- wrap_plots(patchwork_dot, ncol = 2)


### MAKE A scatterplot With Year and Julian Date

install.packages("scatterplot3d") # Install
library("scatterplot3d") # load

###For 3d scatterplot purposes. Change station numbers to be 1,2,3,4,5,6
print(unique(Meso_pred_quantiles$Station))
Station_east_west <- c("NZ028" = 5, "NZ048" = 4, "NZ054" = 3, "NZ060"= 2, "NZ064" = 1, "NZD41" = 6)
Tyler_Meso_pred <- Meso_pred_quantiles
Tyler_Meso_pred$Stations_num <- Station_east_west[Meso_pred_quantiles$Station]
length(Tyler_Meso_pred$Julian_year2)
patchwork_3dot <- c()
stations <- MyZoopsMap %>% ggplot(aes(Longitude, Latitude)) + geom_point(aes(color = Station))

colors <- c('Lower' = "#999999", 'Median' = "#E69F00", 'Upper' = "#56B4E9")
shapes = c("NZ028" = 15, "NZ048" = 16, "NZ054" = 17, "NZ060" = 18, "NZ064" = 19, "NZD41" = 20)
unique(Tyler_Meso_pred$Station)
length(is.na(Tyler_Meso_pred$Quantile))


plot_df <- Tyler_Meso_pred %>% filter(Species == "Limnoithona tetraspina")
patchwork_dot_l <- c()
for (station in unique(plot_df$Station)) {
  plot_df <- plot_df %>% filter(Station == station)
  dot_plot_l <- plot_df %>% ggplot(aes(x=Species_year2 , y=Julian_year2))+
        geom_point(aes(color=Quantile), size=.5)
  patchwork_dot_l[[species]] <- dot_plot_l
}
patchwork_dot_l <- wrap_plots(patchwork_dot_l, ncol = 3)


print(species)
patchwork_3dot <- wrap_plots(patchwork_3dot, ncol = 2)




Tyler_meso_df <- meso_df
unique(Tyler_meso_df$Species)

view(Tyler_meso_df)

### PLOT OvER TRUTH Limnoithona tetraspina ###
Tyler_meso_df <- meso_df
Tyler_meso_df <- Tyler_meso_df %>% filter(Species == "Limnoithona tetraspina")
og_limno <- ggplot(Tyler_meso_df, aes(Date, CPUE))+
  geom_vline(aes(xintercept=Year), color="gray50")+
  geom_line(col="black", linewidth=1)+
  # geom_point(data=Meso_pred_quantiles_Limno, aes(Date, CPUE_pred0, col=Quantile))+
  scale_x_date(breaks="1 year")+
  theme_classic()+
  theme(axis.text.x = element_text(angle=90))+
  facet_grid(Station~Species, scales="free_y")+
  labs(y="CPUE", title="Limnoithona tetraspina", subtitle="Original Data(Monthly)")

patchwork_Limno <- wrap_plots(og_limno, loess_pred_Limno, nrow = 2)

### PLOT OVER TRUTH Bosmina longirostris###
Tyler_meso_df <- meso_df
Tyler_meso_df <- Tyler_meso_df %>% filter(Species == "Bosmina longirostris")
og_Bos <- ggplot(Tyler_meso_df, aes(Date, CPUE))+
  geom_vline(aes(xintercept=Year), color="gray50")+
  geom_line(col="black", linewidth=1)+
  #geom_point(data=Meso_pred_quantiles_Bos, aes(Date, CPUE_pred0, col=Quantile))+
  scale_x_date(breaks="1 year")+
  ...
  theme(axis.text.x = element_text(angle=90))+
  facet_grid(Station~Species, scales="free_y")+
  labs(y="CPUE", title="Bosmina longirostris", subtitle="Original Data(Monthly)")

patchwork_Bos <- wrap_plots(og_Bos, loess_pred_Bos, nrow = 2)


### PLOT OVER TRUTH Pseudodiaptomus forbesi###
Tyler_meso_df <- meso_df
Tyler_meso_df <- Tyler_meso_df %>% filter(Species == "Pseudodiaptomus forbesi")
og_pseudo <- ggplot(Tyler_meso_df, aes(Date, CPUE))+
  geom_vline(aes(xintercept=Year), color="gray50")+
  geom_line(col="black", linewidth=1)+
  geom_point(data=Meso_pred_quantiles_Pseudo, aes(Date, CPUE_pred0, col=Quantile))+
  scale_x_date(breaks="1 year")+
  ...
  theme(axis.text.x = element_text(angle=90))+
  facet_grid(Station~Species, scales="free_y")+
  labs(y="CPUE", title="Pseudodiaptomus forbesi", subtitle="Original Data(Monthly)")

patchwork_pseudo <- wrap_plots(og_pseudo, loess_pred_pseudo, nrow = 2)


### PLOT OVER TRUTH Sinocalanus doerrii###
Tyler_meso_df <- meso_df
Tyler_meso_df <- Tyler_meso_df %>% filter(Species == "Sinocalanus doerrii")
og_sino <- ggplot(Tyler_meso_df, aes(Date, CPUE))+
  geom_vline(aes(xintercept=Year), color="gray50")+
  geom_line(col="black", linewidth=1)+
  geom_point(data=Meso_pred_quantiles_Sino, aes(Date, CPUE_pred0, col=Quantile))+
  scale_x_date(breaks="1 year")+
  ...
  theme(axis.text.x = element_text(angle=90))+
  facet_grid(Station~Species, scales="free_y")+
  labs(y="CPUE", title="Sinocalanus doerrii", subtitle="Original Data(Monthly)")

patchwork_Sino <- wrap_plots(og_sino, loess_pred_Sino, nrow = 2)



patchwork_temp_sal2 <- c()
Tyler_meso_df <- meso_df
plot_df <- Tyler_meso_df %>% filter(Species == "Sinocalanus doerrii")

Temp_plot <- ggplot(plot_df, aes(Date, Temperature))+
  geom_vline(aes(xintercept=Year), color="gray50")+
  geom_line(col="black", linewidth=1)+
  scale_x_date(breaks="1 year")+
  theme_classic()+
  theme(axis.text.x = element_text(angle=90))+
  facet_grid(Station~Species, scales="free_y")+
  labs(y="Temperature", title="Temperature TimeSeries", subtitle="Original Data(Monthly)")

patchwork_temp_sal2[["Temperature"]] <- Temp_plot


salinity_bot_plot <- ggplot(plot_df, aes(Date, SalBott))+
  geom_vline(aes(xintercept=Year), color="gray50")+
  geom_line(col="black", linewidth=1)+
  scale_x_date(breaks="1 year")+
  theme_classic()+
  theme(axis.text.x = element_text(angle=90))+
  facet_grid(Station~Species, scales="free_y")+
  labs(y="Salinity Bottom", title="Salinity Bottom TimeSeries", subtitle="Original Data(Monthly)")

patchwork_temp_sal2[["Salinity Bottom"]] <- salinity_bot_plot

salinity_surf_plot <- ggplot(plot_df, aes(Date, SalSurf))+
  geom_vline(aes(xintercept=Year), color="gray50")+
  geom_line(col="black", linewidth=1)+
  scale_x_date(breaks="1 year")+
  theme_classic()+
  theme(axis.text.x = element_text(angle=90))+
  facet_grid(Station~Species, scales="free_y")+
  labs(y="Salinity Bottom", title="Salinity Surf TimeSeries", subtitle="Original Data(Monthly)")

patchwork_temp_sal2[["Salinity Surf"]] <- salinity_surf_plot

patchwork_temp_sal2 <- wrap_plots(patchwork_temp_sal2)



#### TYLER TO RECREATE HER LOESS EXAMPLES WHILE OVERLAYING THE ACTUAL COLLECTION CPUE

Tylers_Meso_loess_pred3 <- Meso_loess_pred3
Tyler_meso_df_to_join <- Tyler_meso_df %>% select("Station", "Date", "Species", "CPUE", "Taxname", "SalSurf", "SalBott", "Secchi", "Temperature")
Tylers_Meso_loess_pred3 <- Tylers_Meso_loess_pred3 %>% left_join(Tyler_meso_df_to_join, by=c("Station", "Date", "Species"))
view(Tylers_Meso_loess_pred3)
view(Tyler_meso_df_to_join)



# Sino example
Tylers_Meso_loess_pred3_sino <- Tylers_Meso_loess_pred3 %>%
  filter(Species=="Sinocalanus doerrii") #%>% 
  #filter(Station != "NZD41")
view(Tylers_Meso_loess_pred3_sino)
Meso_pred_quantiles_Sino <- Meso_pred_quantiles1 %>%
    filter(Species=="Sinocalanus doerrii")  #%>% 
  #filter(Station != "NZD41")
...
loess_pred_Sino <- ggplot(Tylers_Meso_loess_pred3_sino, aes(Date, CPUE_pred0))+
  geom_vline(aes(xintercept=Species_year3), color="gray50")+
  geom_line(col="black", linewidth=1)+
...
Tyler_Mapping_df <- MyZoopsMap
Tyler_Mapping_df$Region <- c('Confluence', 'Delta', 'Suisun Bay', 'Suisun Bay', 'Confluence', 'San Pablo Bay')
center_lat <- sum(Tyler_Mapping_df$Latitude)/6
center_long <- sum(Tyler_Mapping_df$Longitude)/6
map <- leaflet::leaflet() %>%
  addProviderTiles("Esri.WorldStreetMap") %>%
  setView(lng = center_long, lat = center_lat, zoom = 8)

# Check if data is not empty


map <- map %>%
  addCircleMarkers(data = Tyler_Mapping_df,
    lng = ~Longitude,
    lat = ~Latitude,
    label = ~paste0(" Station: ", Station, " || Region: ", Region), labelOptions = labelOptions(textsize = "18px"),
    # color = Station,
    radius = 7,
    fillOpacity = 1,
    stroke = FALSE
